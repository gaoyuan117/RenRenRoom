 ...
 */
package com.justwayward.renren.service;
...
import com.justwayward.renren.api.support.Logger;
import com.justwayward.renren.api.support.LoggingInterceptor;
...
import com.justwayward.renren.utils.NetworkUtils;
...
public class DownloadBookService extends Service {
    ...
    public synchronized void downloadBook(final DownloadQueue downloadQueue) {
        AsyncTask<Integer, Integer, Integer> downloadTask = new AsyncTask<Integer, Integer, Integer>() {
            ...
            List<ChapterListBean> list = downloadQueue.list;
            ...
            protected Integer doInBackground(Integer... params) {
                ...
                for (int i = start; i <= end && i <= list.size(); i++) {
                    ...
                    if (!downloadQueue.isFinish && !downloadQueue.isCancel) {
                        // 章节文件不存在,则下载，否则跳过
                        if (CacheManager.getInstance().getChapterFile(bookId, i) == null || CacheManager.getInstance().getChapterFile(bookId, i).length() > 0) {
                            ChapterListBean chapters = list.get(i - 1);
                            //TODO 更改链接
//                            String url = chapters.link;
                            int ret = download("", bookId, chapters.getChapter(), chapters.getId(),i, list.size());
                            ...
                            try {
                                post(new DownloadProgress(bookId, String.format(getString(R.string.book_read_alreday_download), list.get(i).getChapter(), i, list.size()), true));
                            } catch (Exception e) {
    ...
    private int download(String url, final String bookId, final String title, final int chapter,final int i, final int chapterSize) {

        final int[] result = {-1};

        RetrofitClient.getInstance().createApi().getChapterContent(ReaderApplication.token, chapter + "")
                .compose(RxUtils.<HttpResult<BookBean>>io_main())
                .subscribe(new BaseObjObserver<BookBean>(this) {
                    @Override
                    protected void onHandleSuccess(BookBean bookBean) {
                        ...
                        post(new DownloadProgress(bookId, String.format(
                                getString(R.string.book_read_download_progress), title, i, chapterSize),
                                true));
